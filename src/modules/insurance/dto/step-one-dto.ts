...
export class stepOneRequestDTO implements StepOneRequest {
  ...
  @IsNotEmpty()
  @Length(3, 3)
  @Matches(/^[A-Z]{3}$/, { message: 'texpsery must be exactly 3 uppercase letters' })
  texpsery: string
  ...
  @IsNotEmpty()
  @Length(8, 8)
  @Matches(/^\d{2}[A-Z]\d{3}[A-Z]{2}$|^\d{5}[A-Z]{3}$/, { message: 'renumber must match the specified pattern' })
  renumber: string
}
